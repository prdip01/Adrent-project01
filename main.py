...
def load_data():
    try:
        ...
        if 'v1' in data.columns and 'v2' in data.columns:
            data = data[['v1', 'v2']].rename(columns={'v1': 'Category', 'v2': 'Message'})
...
st.markdown("<p style='text-align: center; color: #666;'>Detect spam email messages with AIML</p>", unsafe_allow_html=True)

if welcome_lottie:
    st_lottie(welcome_lottie, height=200, key="welcome")
...
with st.container():
    ...
    with col2:
        ...
        spam_count = data['Category'].value_counts().get('Spam', 0)
        spam_ratio = round(spam_count / len(data) * 100, 1)
        st.metric("Accuracy", f"{round(model.score(X_test_tfidf, y_test) * 100, 2)}%")
        st.metric("Total Samples", len(data))
        st.metric("Spam Ratio", f"{spam_ratio}%")
